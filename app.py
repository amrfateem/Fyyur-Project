...
#commened Json cause iv VS code it shows as not being used. could be wrong.
...
# Said to replace data but since it's on a local database i don't see the point.
# but adding data in the site it self works well and updates the database
    ...
    for area in all_areas:
        ...
        for venue in area_venues:
            venue_data.append({
                "id": venue.id,
                "name": venue.name,
                "ShowsUp": len(db.session.query(Show).filter(Show.venue_id == 1).filter(Show.start_time > datetime.now()).all())
            })
        data.append({
            "city": area.city,
            "state": area.state,
...
#didn't test the search but it should work when there is some data
...
def search_venues():
    ...
    for result in search_result:
        data.append({
            "id": result.id,
            "name": result.name,
            "ShowsUp": len(db.session.query(Show).filter(Show.venue_id == result.id).filter(Show.start_time > datetime.now()).all()),
        })
...
def search_artists():
    ...
    for result in search_result:
        data.append({
            "id": result.id,
            "name": result.name,
            "ShowsUp": len(db.session.query(Show).filter(Show.artist_id == result.id).filter(Show.start_time > datetime.now()).all()),
        })
